...
/**
 * Collects local client player / container state into one pretty-printed JSON object.
 */
public final class PlayerDataCollector {
    ...
    public static String collectJsonLine() {
        ...
        if (mc.getUser() != null) {
            ...
            game.addProperty("accountUuid", mc.getUser().getGameProfile().getId().toString());
        }
        ...
        p.addProperty("biome", level.getBiome(blockPos).unwrapKey()
        ...
        p.addProperty("exhaustion", player.getFoodData().getExhaustionLevel());
        ...
        p.add("inventory", mainInv);

        JsonArray offhandArr = new JsonArray();
        for (int i = 0; i < inv.offhand.size(); i++) {
            JsonObject slot = itemToJson(inv.offhand.get(i));
            slot.addProperty("slot", i);
            offhandArr.add(slot);
        }
        p.add("offhandInventory", offhandArr);

        JsonArray armorInv = new JsonArray();
        for (int i = 0; i < inv.armor.size(); i++) {
            JsonObject slot = itemToJson(inv.armor.get(i));
            slot.addProperty("slot", i);
            armorInv.add(slot);
        }
        p.add("armorInventory", armorInv);
    ...
    private static JsonObject effectToJson(MobEffectInstance effect) {
        ...
        obj.addProperty("id", key != null ? key.toString() : "unknown");
    ...
    private static JsonObject itemToJson(ItemStack stack) {
        ...
        if (map.isEmpty() && stack.getItem() instanceof EnchantedBookItem) {
            map = EnchantmentHelper.getEnchantments(stack);
        }
        for (Map.Entry<Enchantment, Integer> entry : map.entrySet()) {
            JsonObject e = new JsonObject();
            ResourceLocation ek = ForgeRegistries.ENCHANTMENTS.getKey(entry.getKey());
            e.addProperty("id", ek != null ? ek.toString() : "unknown");
